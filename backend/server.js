...
pool.query('SELECT NOW()', (err, res) => {
  if (err) {
    console.error('Γ¥î Database connection error:', err);
  } else {
    console.log('Γ£à Database connected at:', res.rows[0].now);
  }
});
...
wss.on('connection', (ws) => {
  console.log('≡ƒöî New WebSocket client connected');
  
  ws.on('message', (message) => {
    try {
      const data = JSON.parse(message);
      console.log('≡ƒô¿ Received:', data);
  ...
  ws.on('close', () => {
    console.log('≡ƒöî Client disconnected');
  });
});
...
server.listen(PORT, () => {
  ...
  console.log(`≡ƒôí WebSocket server ready`);
  console.log(`≡ƒîÉ Environment: ${process.env.NODE_ENV || 'development'}`);
});
